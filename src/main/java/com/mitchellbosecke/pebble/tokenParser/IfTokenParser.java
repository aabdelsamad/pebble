...
public class IfTokenParser extends AbstractTokenParser {
	...
	public RenderableNode parse(Token token, Parser parser) throws ParserException {
		...
		List<Pair<Expression<?>, BodyNode>> conditionsWithBodies = new ArrayList<Pair<Expression<?>, BodyNode>>();
		...
		boolean end = false;
		while (!end) {
			String streamCurrentvalue = stream.current().getValue();

			if (streamCurrentvalue.equals("else")) {
				stream.next();
				stream.expect(Token.Type.EXECUTE_END);
				elseBody = parser.subparse(decideIfEnd);
			} else if (streamCurrentvalue.equals("elseif")) {
				stream.next();
				expression = parser.getExpressionParser().parseExpression();
				stream.expect(Token.Type.EXECUTE_END);
				body = parser.subparse(decideIfFork);
				conditionsWithBodies.add(new Pair<Expression<?>, BodyNode>(expression, body));
			} else if (streamCurrentvalue.equals("endif")) {
				stream.next();
				end = true;
			} else {
