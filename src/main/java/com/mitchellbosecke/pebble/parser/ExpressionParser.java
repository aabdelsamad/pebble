...
public class ExpressionParser {
	...
	private Expression<?> parseExpression(int minPrecedence) throws ParserException {
		...
		while (isBinary(token) && binaryOperators.get(token.getValue()).getPrecedence() >= minPrecedence) {
			...
			// unique way
				 ...
				 */
				expressionRight = parseExpression(Associativity.LEFT.equals(operator.getAssociativity())
	...
	private Expression<?> subparseExpression() throws ParserException {
		...
		case NAME:
			...
			}else if(tokenValue.equalsIgnoreCase("false")){
				node = new LiteralBooleanExpression(false, token.getLineNumber());
			}else if (tokenValue.equalsIgnoreCase("none") || tokenValue.equalsIgnoreCase("null")){
		...
		default:
			throw new ParserException(null, String.format("Unexpected token \"%s\" of value \"%s\"",
					token.getType().toString(), token.getValue()), token.getLineNumber(), stream.getFilename());
		}
	...
	private Expression<?> parseTernaryExpression(Expression<?> expression) throws ParserException {
		...
		expression = new TernaryExpression((Expression<Boolean>) expression, expression2, expression3,
				this.stream.current().getLineNumber(), stream.getFilename());
		return expression;
	}
	...
	private Expression<?> parseFunctionOrMacroInvocation(Expression<?> node) throws ParserException {
		...
		} else if (functionName.equals("block")) {
	...
	private Expression<?> parseBeanAttributeExpression(Expression<?> node) throws ParserException {
		...
		if (stream.current().test(Token.Type.PUNCTUATION, ".")) {
			...
			node = new GetAttributeExpression(node,
					new LiteralStringExpression(token.getValue(), token.getLineNumber()), args, stream.getFilename(),
					token.getLineNumber());
	...
	private Expression<?> parseMapDefinitionExpression() throws ParserException {
		...
		Map<Expression<?>, Expression<?>> elements = new HashMap<Expression<?>, Expression<?>>();
		while (true) {
			// key : value
